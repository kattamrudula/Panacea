...
jsonform.elementTypes = {
  ...
  'doublerange':{
    ...
    ' aria-label="<%= node.title ? escape(node.title) : node.name %>"' +
    '<%= (node.disabled? " disabled" : "")%>' +
    '<%= (node.readOnly ? " readonly=\'readonly\'" : "") %>/><span style="margin:10px;position:relative" class="custrange-value" rel="<%= id %>1"><%= escape(value) %></span><div id="<%= id %>1">'+
    '</div>',
    'fieldtemplate': true,
    'inputfield': true,
    'onInsert': function (data, node) {
      $("#" + node.id).val(node.schemaElement.defaultMinimum + " - "+  node.schemaElement.defaultMaximum)
      $("#" + node.id+"1").slider({
        range: true,
        min: node.schemaElement.minimum,
        max: node.schemaElement.maximum,
        values: [node.schemaElement.defaultMinimum, node.schemaElement.defaultMaximum],
        slide: function (event, ui) {
          debugger
          const valueIndicator = document.querySelector('span.custrange-value[rel="' + event.currentTarget.id + '"]');
          if (valueIndicator) {
            valueIndicator.innerText = ui.values[0] + " - " + ui.values[1];
          }
          $("#" + node.id).val(ui.values[0] + " - " + ui.values[1]);
        }
      });
      const indicator = document.querySelector('span.custrange-value[rel="' + node.id + '1"]');
      if (indicator) {
        let defVal = $("#" + node.id)[0].defaultValue;
        if(defVal != "false"){
          indicator.innerText = $("#" + node.id)[0].defaultValue;
          //console.log($("#" + node.id)[0].defaultValue.split(" - "));
          $("#" + node.id+"1").slider('values',0,parseInt($("#" + node.id)[0].defaultValue.split(" - ")[0]))
          $("#" + node.id+"1").slider('values',1,parseInt($("#" + node.id)[0].defaultValue.split(" - ")[1]))
        }
        else{
          indicator.innerText = $("#" + node.id).val();
          $("#" + node.id+"1").slider('values',0, parseInt($("#" + node.id).val().split(" - ")[0]));
          $("#" + node.id+"1").slider('values',1, parseInt($("#" + node.id).val().split(" - ")[1]));
        }
      }
    }
  },
